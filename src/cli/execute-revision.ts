...
export async function executeRevision(args: string[]): Promise<void> {
  ...
  if (!jobDir) {
    throw new Error('execute-revision requires --job <dir> or --latest');
  }
  ...
  if (tmuxPane || tmuxSession) {
    const existing = await readModeState(projectDir, 'revision');
  ...
  const summary = await executeRevisionJob({
    jobDir,
    codexCmd,
    model,
    profile,
  ...
  console.log(`Executed revision job: ${summary.jobDir}`);
  console.log(`Revision phases: ${summary.phases.length}`);
}
