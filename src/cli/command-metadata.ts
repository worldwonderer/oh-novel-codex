...
export const CLI_COMMAND_DEFINITIONS: readonly CliCommandDefinition[] = [
  ...
  { id: 'mcp-server', command: 'mcp-server', syntax: 'onx mcp-server [all|state|memory|trace|team]', summary: 'Launch a stdio MCP server for ONX surfaces.', category: 'runtime' },
  { id: 'mcp-config', command: 'mcp-config', syntax: 'onx mcp-config [--surface all|state|memory|trace|team] [--node <path>] [--onx <path>] [--output <file>]', summary: 'Generate MCP client config JSON for ONX servers.', category: 'runtime' },
